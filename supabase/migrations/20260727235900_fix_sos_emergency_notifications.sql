-- Repair migration for SOS/emergency notifications.
-- Keep this separate from 20260726140156 because already-applied Supabase
-- migrations are not re-run when the original file is edited.
...
DROP POLICY IF EXISTS "Household members can view emergency contacts" ON public.emergency_contacts;
...
DROP POLICY IF EXISTS "Household members can insert emergency contacts" ON public.emergency_contacts;
...
DROP POLICY IF EXISTS "Household members can update emergency contacts" ON public.emergency_contacts;
CREATE POLICY "Household members can update emergency contacts"
  ON public.emergency_contacts FOR UPDATE TO authenticated
  USING (public.is_household_member(household_id, auth.uid()))
  WITH CHECK (public.is_household_member(household_id, auth.uid()));

DROP POLICY IF EXISTS "Household members can delete emergency contacts" ON public.emergency_contacts;
...
DROP POLICY IF EXISTS "Household members can view SOS events" ON public.sos_events;
CREATE POLICY "Household members can view SOS events"
  ON public.sos_events FOR SELECT TO authenticated
  USING (public.is_household_member(household_id, auth.uid()));

DROP POLICY IF EXISTS "Household members can create SOS events" ON public.sos_events;
...
DROP POLICY IF EXISTS "Admins can delete SOS events" ON public.sos_events;
...
CREATE INDEX IF NOT EXISTS idx_emergency_contacts_household
  ON public.emergency_contacts (household_id);
