...
async function scanWithGemini(imageUrl: string): Promise<Receipt> {
  ...
  const models = Array.from(new Set([configuredModel, "gemini-3.6-flash", "gemini-3-flash-preview"].filter(Boolean)));
  ...
  if (!response?.ok) {
    throw new Error(
      `Gemini OCR respondió ${response?.status ?? "sin respuesta"}${lastError ? `: ${lastError.slice(0, 220)}` : ""}`,
    );
  }
