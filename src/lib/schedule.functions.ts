...
const UpsertSettingsInput = z.object({
  ...
  vacation_balance_adjustment: z.number().min(-365).max(365).optional(),
  vacation_start_date: z.string().nullable().optional(),
...
export const upsertScheduleSettings = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const payload: any = {
      ...
      ...(data.vacation_balance_adjustment !== undefined ? { vacation_balance_adjustment: data.vacation_balance_adjustment } : {}),
      ...(data.vacation_start_date !== undefined ? { vacation_start_date: data.vacation_start_date || null } : {}),
