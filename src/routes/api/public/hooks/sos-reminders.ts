    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        const { data: events, error } = await supabase
          .from("sos_events")
          .select(
            "id, household_id, triggered_by, triggered_by_name, latitude, longitude, location_accuracy, note, created_at, acknowledged_at, last_reminder_sent_at, reminder_count, is_test, sos_type, battery_level, battery_charging, connection_type, location_source, last_known_location_used",
          )
