...
export async function connectAppUser(opts: {
  ...
  if (!popup) return { success: false, connectorId, error: "El navegador ha bloqueado la ventana emergente. Permítela y vuelve a intentarlo." };
  ...
  try {
    ...
    popup.close();
    return { success: false, connectorId, error: e instanceof Error ? e.message : "No se pudo iniciar OAuth" };
  }
  ...
  return await new Promise<AppUserOAuthResult>((resolve) => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      resolve({ success: false, connectorId, error: data.error ?? "OAuth falló" });
    };
    window.addEventListener("message", onMessage);
    const timer = setInterval(() => {
      if (popup.closed) {
        cleanup();
        resolve({ success: false, connectorId, error: "Se canceló el inicio de sesión" });
      }
    }, 500);
  });
}
